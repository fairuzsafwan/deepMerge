...
trainDataFiles, validationDataFiles, testDataFiles = commonFuncs.obtainDataPath(opt.benchmark, opt.testPhase, true, opt.dataset)
local allData = {}
...
elseif opt.expType == 'forwardPass' then
	...
	if opt.forwardPassType == 'userData' then
		...
		print ("==> The results will be stored at '" .. experimentResultOutputPath .. "'")
		local dataTensor = commonFuncs.loadUserDataForDropoutNet('ExtraData/userData', opt.forwardPassType, opt.numVPs, opt.silhouetteInput)
		for i=1, dataTensor:size(2) do
			image.save(string.format('test%d.png', i), dataTensor[1][i])
		end
		print ("--> Checkpoint 1")
		dataTensor = dataTensor:cuda()
		local silhouettes = dataTensor:clone()
		if opt.tanh then
		...
		local tempDepthImg = torch.cat(dataTensor[{{i}}], dataTensor[{{i}}], 1) -- This resolve the batch normalization issue
		...
		--	tempTensor[i] = torch.cat(dataTensor[{{i}}], dataTensor[{{i}}], 1)
			-- local tempTensor = torch.cat(dataTensor, dataTensor, 1)
		--end
		print ("--> Checkpoint 2")
		local vLimit = {}
		local vChosen = {}
		...
		droppedInputs = commonFuncs.dropInputVPs({tempDepthImg, tempSilImg}, true, opt.dropoutNet, numOfVPsToDrop, dropIndices, opt.singleVPNet, pickedVPs, vLimit, vChosen)
		
		local networkInput = {}
		for j=1, 2 do -- j == 1 for depth maps and j == 2 for silhouettes
			local temp2 = droppedInputs[j][{{1}}]:clone()
			temp2 = torch.cat(temp2, temp2, 1)
			networkInput[j] = temp2
		end
		
		
		local outputPath = experimentResultOutputPath .. '/model' .. (opt.conditional and '-predictedClass-' .. data.category[temp[1][1]] or '-userData')
		paths.mkdir(outputPath .. '/viewpointsChosen')
		
		-- print ("**** dropped views 2 ****")
		-- for u=1, #vLimit do
		--	print ("Length:'" .. vLimit[u] .. "'")
		--	for i=1, #vChosen[u] do
		--		print ("****'" .. vChosen[u][i] .. "'")
		--	end
		--end
		...
		if opt.dropoutNet then
			print ("-------dropoutNet--------")
			for j=1, #vChosen[#vLimit] do
				-- print ()
				image.save(outputPath .. '/viewpointsChosen/img-' .. vChosen[#vLimit][j]-1 .. '-VC.png', dataTensor[1][vChosen[#vLimit][j]])
				-- droppedSilhouettesTensor[i][dropIndices[j]]:zero()
			end
		elseif opt.singleVPNet then
			print ("Code not modified yet!!!")
		else
			print ("-------AllVPNet--------")
			print ("--> Using all viewpoints loaded!")
			
			if dataTensor:size(2) > opt.numVPs  then
				print ("number of loaded images larger than required viewpoints ('" .. opt.numVPs .. "')")
			elseif dataTensor:size(2) < opt.numVPs then
				print ("number of loaded images smaller than required viewpoints ('" .. opt.numVPs .. "')")
			else
				for j=1, dataTensor:size(2) do
					image.save(outputPath .. '/viewpointsChosen/img-' .. j-1 .. '-VC.png', dataTensor[1][j])
					image.save(outputPath .. '/file1-img-' .. j-1 .. '-or.png', dataTensor[1][j])
					image.save(outputPath .. '/mask/file1-img-' .. j-1 .. '-or.png', silhouettes[1][j])
				end
			end
		end
		
		-- droppedInputs = commonFuncs.dropInputVPs(dataTensor, true, opt.dropoutNet, nil, nil, opt.singleVPNet)
        if opt.conditional then
        ...
        for k=1, recon[1]:size(2) do
            image.save(outputPath .. '/file1-img-' .. k-1 .. '-rec.png', recon[1][1][k])
            image.save(outputPath .. '/mask/file1-img-' .. k-1 .. '-rec.png', recon[2][1][k])
        end
			...
			for l=1, opt.singleVPNet and opt.allViewsExp and opt.numVPs or 1 do
				...
				for i=1, #dataFilePaths do
					...
					for j=1, data.dataset:size(1) do
		                ...
		                local dropIndices = torch.zeros(opt.numVPs) -- A placeholder to hold the indices of the tensor to be zeroed-out  -- Used for dropoutNet
		                local pickedVPs = torch.Tensor(2) -- A placeholder to hold the view point to be kept -- Used for singleVPNet
		                if not opt.allViewsExp and opt.VpToKeep >= opt.numVPs then
		                    pickedVPs[1] = torch.random(1, opt.numVPs)
		                    pickedVPs[2] = pickedVPs[1]
		                else
						...
						depthMaps = torch.cat(depthMaps, depthMaps, 1) -- fed twice
						local silhouettes = depthMaps:clone()
						if opt.tanh then
				            silhouettes[silhouettes:gt(-1)] = 1
				            silhouettes[silhouettes:eq(-1)] = 0
