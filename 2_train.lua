...
require 'nngraph'
require 'optim'
--local VAE = require opt.VAE --'2_0_VAE16_v13'
...
if not opt then
	...
	--cmd:option('-globalDataType', 'float', 'Sets the default data type for Torch tensors: float, double')
	cmd:option('-dataset', '', 'The dataset to be used: 57 or 54 catergories, oriCamPos or ownCamPos')
	cmd:option('-VAE', '', 'The VAE file to be used.')
	...
	if opt.dataset == '' then print "==> Please include a -dataset argument and determine a dataset to be used." os.exit() end
	if opt.VAE == '' then print "==> Please include a -VAE argument and determine a VAE file to be used." os.exit() end
...
--if pcall(require, vaeFile) then
--    local VAE = require, opt.VAE
--    print ("VAE included")
--else
--    print '==> Error: VAE not included!'
--end
...
local trainDataFiles, validationDataFiles, testDataFiles = commonFuncs.obtainDataPath(opt.benchmark, opt.testPhase, true, opt.dataset)
...
local inputTensorSize = {data.dataset:size(2), opt.imgSize, opt.imgSize} -- {20,240,240}
    ...
    for i=2, #trainDataFiles + 1 do -- For all training data set files
        ...
        for t,v in ipairs(indices) do
            ...
            local opfunc = function(x)
				...
				--test = model:forward(droppedInputs)
				--print("Test1 : ")
				--print(test:size())
